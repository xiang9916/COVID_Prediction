...
class LstmModel(torch.nn.Module): # 继承自 torch.nn.Module
    ...
    def forward(self, x):
        ...
        y4 = self.relu(y3)
        y5 = self.linear3(y4)
        return y5.view(self.output_seq_length)
...
def dataset_generator(input_seq_length, output_seq_length, pred_length, source) -> tuple([torch.tensor, torch.tensor, torch.tensor, torch.tensor]):
    ...
    for i in range(source_len - input_seq_length - output_seq_length - pred_length):
        ...
        y_data = np.array(source[i+input_seq_length:i+input_seq_length+output_seq_length])
        y_dynamic = np.array(get_label(source_len-(i+input_seq_length), output_seq_length)).T
        y = torch.from_numpy((y_data+y_dynamic)/2).to(torch.float32).reshape(-1)
        x_label = torch.from_numpy(source[i+pred_length:i+input_seq_length+pred_length]).to(torch.float32)
        y_label_data = np.array(source[i+input_seq_length+pred_length:i+input_seq_length+output_seq_length+pred_length])
        y_label_dynamic = np.array(get_label(source_len-(i+input_seq_length+pred_length), output_seq_length)).T
        y_label = torch.from_numpy((y_label_data+y_label_dynamic)/2).to(torch.float32).reshape(-1)
...
def train(input_dims, input_seq_length, output_dims, output_seq_length, pred_length, h_dims, lstm_num_layers, source, subepochs_init, learning_rate) -> None:  
    ...
    model.to('cpu')
    torch.save(model, './data/model/singleLSTMplus_{}*{}_{}*{}_{}_{}.pth'.format(input_dims, input_seq_length, output_dims, output_seq_length, h_dims, subepochs_init)) 

    return test_losses[-1]

def get_label(start: int, length: int):
    label = pd.DataFrame(Dynamic_Model.main(start, length))
    df_new_mean = pd.read_pickle('./data/data/df_new.mean().pkl')['United States of America']
    df_new_std = pd.read_pickle('./data/data/df_new.std().pkl')['United States of America']
    return ((label-df_new_mean)/df_new_std)
...
if __name__ == '__main__':
    device = set_device()
    df = pd.read_csv('./data/data/New_cases_normalized.csv')['United States of America']
    
    for subepochs_init in np.logspace(6,10,5, base=2):
        for learning_rate in np.logspace(1,4,4, base=0.1):
            print('train {} {}'.format(subepochs_init, learning_rate))
            print(train(
                input_dims=1,
                input_seq_length=56,
                output_dims=1,
